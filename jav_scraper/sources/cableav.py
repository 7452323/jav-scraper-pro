...
def scrape(number: str) -> JavMetadata | None:
    ...
    if not match:
        # Try getting any video link
        match = re.search(
            r'<a\\s+href="(https?://cableav\\.tv/[^/]+/\\d+/[^"]+)"',
            html, re.IGNORECASE
        )
    ...
    if title_match:
        ...
        meta.title_jp = re.sub(
            r'\\s*[-–|]\\s*Cable.*$', '', meta.title_jp, flags=re.IGNORECASE
        ).strip()
